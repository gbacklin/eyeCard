...
class ViewController: UIViewController {
    ...
    func displayChosenNumber() {
        let message = "Your number was \(self.chosenNumber!)\nTry again ?"
    ...
    func displayCard(_ index: Int!) {
        UIView.transition(with: self.contentView, duration: 1.0, options: .transitionFlipFromRight, animations: { () -> Void in
            self.contentView.image = UIImage(named: "\(index!)")
            }, completion: nil)
    }
